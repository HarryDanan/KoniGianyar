  ...
  private function get_filtering()
  {
        ...
        for ($i = 0; $i < count($mColArray); $i++) {
          if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns)) {
            if ($this->check_cType()) {
              ...
              //   $temp = $this->select[$mColArray[$i]['data']] . " LIKE '%" . $mColArray[$i]['search']['value'] . "%' OR ";
              //   echo $temp;
              //   $sWhere .= $temp;
              // }
            } else {
    ...
    for ($i = 0; $i < count($mColArray); $i++) {
      if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns)) {
        if ($mColArray[$i]['search']['value'] != '') {
          $flag_search = 1;
          $search_colomn_val = $mColArray[$i]['search']['value'];
          if (preg_match("/^\d+$/", $search_colomn_val)) {
            $temp = $this->select[$mColArray[$i]['data']] . " = '" . $mColArray[$i]['search']['value'] . "' AND ";
            $temp_where .= $temp;
          } else {
            $sSearch = $this->ci->db->escape_like_str(trim($search_colomn_val));
            $sSearch_explode = explode(" ", $sSearch);
            $temp_where .= '(';
            foreach ($sSearch_explode as $sSearch) {
              // echo ($this->select[$mColArray[$i]['data']] . " LIKE '%" . $sSearch . "%' OR ");
              $temp_where .= $this->select[$mColArray[$i]['data']] . " LIKE '%" . $sSearch . "%' AND ";
            }
            $temp_where = substr_replace($temp_where, '', -4);
            $temp_where .= ') AND ';
          }
        }
      }
